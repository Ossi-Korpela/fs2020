...
blogsRouter.get('/', async (request, response) => {
  ...
  blogsRouter.post('/:id/comments', async (request, response) => {
    try{
      ...
      const commentedBlog = {...blog, comments : blog.comments.push(comment)}
      const updated = await Blog.findByIdAndUpdate(request.params.id, commentedBlog).populate('user', {username: 1, name: 1})
